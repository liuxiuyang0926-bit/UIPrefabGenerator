...
- 素材匹配、候选联系图和审计预览的临时相机显式关闭 HDR/MSAA，避免宿主工程的 HDR 中间缓冲丢失 Alpha，导致透明素材漏配、半透明底条错配和艺术字识别失败。无需修改工程颜色空间或游戏相机。
- 增加透明留边、半透明颜色与 UGUI 候选预览回归检查；可使用原 UISchema 重新匹配，无需安装项目 Skill 或重新调用 AI。
